defmodule TodookieWeb.UserAuthTest do
  use TodookieWeb.ConnCase

  alias Phoenix.LiveView
  alias Todookie.Accounts
  alias Todookie.Accounts.Scope
  alias TodookieWeb.UserAuth

  import Todookie.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, TodookieWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      TodookieWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        TodookieWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
